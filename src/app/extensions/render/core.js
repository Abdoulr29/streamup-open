...
function updateTime() {
  ...
  var v = hours + ":" + minutes + "<br/>" + month + "/" + day + "/" + year;
  setTimeout("updateTime()", 1000);
  document.getElementById('system-time').innerHTML = v;
}
updateTime();
...
$(function () {
  ...
  $('.window').click(function (e) {
    ...
    if (!$('.window__close').is(e.target) && $('.window__close').has(e.target).length === 0 && !$(
        '.window__minimize').is(e.target) && $('.window__minimize').has(e.target).length === 0) {
  ...
  $('.window__titlebar').each(function () {
    ...
    $(this).find('.window__close').click(function (e) {
      ...
      $('.taskbar__item[data-window="' + appName + '"]').removeClass('taskbar__item--open').removeClass(
        'taskbar__item--active');

      var closest = $('.window').not('.window--minimized, .window--closing, .window--opening').filter(
        function () {
...
$(document).mouseup(function (e) {
  ...
  if (start.is(':visible') && !startToggle.is(e.target) && startToggle.has(e.target).length === 0 && !start.is(
      e.target) && start.has(e.target).length === 0) {
    toggleStart();
...
$(document).mouseup(function (e) {
  if ($('.menu').has(e.target).length === 0 && !$('.menu-toggle').is(e.target) && $('.menu-toggle').has(e.target)
    .length === 0) {
    $('.menu').hide();
  }
});
